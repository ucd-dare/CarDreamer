...
class CheckSpaces(base.Wrapper):
    ...
    def step(self, action):
        for key, value in action.items():
            ...
            assert value in space, (value.dtype, value.shape, np.min(value), np.max(value), space)
        obs = self.env.step(action)
        for key, value in obs.items():
            space = self.env.obs_space[key]
            assert value in space, (key, np.array(value).dtype, np.array(value).shape, np.min(value), np.max(value), space)
        return obs
