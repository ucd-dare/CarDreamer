...
def lambda_return(reward, value, pcont, bootstrap, lambda_, axis):
    ...
    returns = scan(lambda agg, cur: cur[0] + cur[1] * lambda_ * agg, (inputs, pcont), bootstrap, static=True, reverse=True)
    if axis != 0:
...
class TwoHotDist(tfd.Distribution):
    def __init__(self, dist1, dist2):
        super(TwoHotDist, self).__init__(dtype=tf.int32, reparameterization_type=tfd.NOT_REPARAMETERIZED, validate_args=False, allow_nan_stats=True)
        self.dist1 = dist1
        self.dist2 = dist2
...
class AutoAdapt(Module):
    ...
    def __call__(self, reg, update=True):
        ...
        metrics = {"mean": reg.mean(), "std": reg.std(), "scale_mean": scale.mean(), "scale_std": scale.std()}
        return loss, metrics
