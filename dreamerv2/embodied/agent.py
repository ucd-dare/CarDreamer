...
class Agent(tfagent.TFAgent):
    ...
    def policy(self, obs, state=None, mode="train"):
        ...
        if mode == "eval":
            ...
            outs["log_entropy"] = outs["action"].entropy()
            outs = {**outs, "action": outs["action"].sample()}
        outs = {**outs, "action": tfutils.action_noise(outs["action"], noise, self.act_space)}
...
class ImagActorCritic(tfutils.Module):
    ...
    def __init__(self, critics, scales, act_space, config):
        ...
        self.config = config
        self.actor = nets.MLP(act_space.shape, **self.config.actor, dist=(config.actor_dist_disc if act_space.discrete else config.actor_dist_cont))
...
class TwinQFunction(tfutils.Module):
    ...
    def target(self, traj, actor, reward):
        ...
        value = tf.math.reduce_min(
            [self.target_net1({**traj, "action": actor(traj).sample()}).mean(), self.target_net2({**traj, "action": actor(traj).sample()}).mean()], 0
        )
        if self.config.pengs_qlambda:
            vals = [value[-1]]
            interm = reward + disc * value[1:] * (1 - self.config.return_lambda)
