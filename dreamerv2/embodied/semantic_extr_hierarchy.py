...
class SemanticExtrHierarchy(tfutils.Module):

    def __init__(self, wm, act_space, config):
        self.wm = wm
        self.config = config
        # Environment rewards for manager
        self.extr_reward = lambda traj: self.wm.heads['reward'](traj).mean()[1:]
        if 'goal_completion' in self.wm.heads['decoder'].shapes:
            def goal_reward(traj):
                return sg(self.wm.heads['decoder'](traj)['goal_completion'].mean()[1:])
            self.goal_reward = lambda traj: tf.squeeze(goal_reward(traj), axis=-1)
        else:
            raise NotImplementedError('goal_reward')
    ...
    def heuristic_goal_reward(self, traj):
        feat = self.feat(traj).astype(tf.float32)
