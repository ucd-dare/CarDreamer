...
class RSSM(tfutils.Module):
    ...
    def initial(self, batch_size):
        ...
        if self._initial == "zeros":
            ...
            state["deter"] = tf.repeat(
                self._cast(self.get("initial_deter", tf.Variable, state["deter"][0].astype(tf.float32), trainable=True))[None], batch_size, 0
            )
            state["stoch"] = tf.repeat(
                self._cast(self.get("initial_stoch", tf.Variable, state["stoch"][0].astype(tf.float32), trainable=True))[None], batch_size, 0
            )
            return state
            ...
            # training loop. Afterwards, the state is reset inside the obs_step().
            state["deter"] = tf.repeat(
                self._cast(tf.math.tanh(self.get("initial_deter", tf.Variable, state["deter"][0].astype(tf.float32), trainable=True)))[None],
                batch_size,
                0,
            )
    ...
    def obs_step(self, prev_state, prev_action, embed, is_first):
        ...
        prev_state, prev_action = tf.nest.map_structure(lambda x: tf.einsum("b...,b->b...", x, 1.0 - is_first), (prev_state, prev_action))
        prev_state = tf.nest.map_structure(
            lambda x, y: x + tf.einsum("b...,b->b...", self._cast(y), is_first), prev_state, self.initial(len(is_first))
        )
