...
class Prioritized(dm2.Replay):

    def __init__(self, store, chunk=64, prio_starts=0.0, prio_ends=1.0, sync=0, fraction=0.1, softmax=False, temp=1.0, constant=0.0, exponent=0.5):
    ...
    def add_traj(self, traj):
        ...
        traj = {k: dm2.convert(v) for k, v in traj.items()}
        key = uuid.uuid4().hex
        self.store[key] = traj
