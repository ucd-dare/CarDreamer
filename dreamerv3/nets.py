...
class RSSM(nj.Module):
    ...
    def __init__(self, deter=1024, stoch=32, classes=32, unroll=False, initial="learned", unimix=0.01, action_clip=1.0, **kw):
        self._deter = deter
        self._stoch = stoch
...
class MLP(nj.Module):
    ...
    def __init__(self, shape, layers, units, inputs=["tensor"], dims=None, symlog_inputs=False, **kw):
...
class Dist(nj.Module):
    ...
    def __init__(self, shape, dist="mse", outscale=0.1, outnorm=False, minstd=1.0, maxstd=1.0, unimix=0.0, bins=255):
...
class Conv2D(nj.Module):
    ...
    def __init__(
        self, depth, kernel, stride=1, transp=False, act="none", norm="none", pad="same", bias=True, preact=False, winit="uniform", fan="avg"
    ):
        self._depth = depth
        self._kernel = kernel
        self._stride = stride
    ...
    def _layer(self, x):
        if self._transp:
            ...
            x = jax.lax.conv_transpose(x, kernel, (self._stride, self._stride), self._pad, dimension_numbers=("NHWC", "HWOI", "NHWC"))
        else:
            shape = (self._kernel, self._kernel, x.shape[-1], self._depth)
            kernel = self.get("kernel", Initializer(self._winit, fan=self._fan), shape)
            kernel = jaxutils.cast_to_compute(kernel)
            x = jax.lax.conv_general_dilated(x, kernel, (self._stride, self._stride), self._pad, dimension_numbers=("NHWC", "HWIO", "NHWC"))
        if self._bias:
...
class Linear(nj.Module):
    ...
    def __init__(self, units, act="none", norm="none", bias=True, outscale=1.0, outnorm=False, winit="uniform", fan="avg"):
