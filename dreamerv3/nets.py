...
def sg(x): return tree_map(jax.lax.stop_gradient, x)
...
class RSSM(nj.Module):
    ...
    def initial(self, bs):
        ...
        if self._initial == 'zeros':
            ...
            deter = self.get('initial', jnp.zeros,
                             state['deter'][0].shape, f32)
    ...
    def observe(self, embed, action, is_first, state=None):
        def swap(x): return x.transpose([1, 0] + list(range(2, len(x.shape))))
        if state is None:
    ...
    def rep_loss(self, post, prior, impl='kl', free=1.0):
        if impl == 'kl':
            ...
            uniform = jax.tree_util.tree_map(
                lambda x: jnp.zeros_like(x), prior)
    ...
    def __init__(
        ...
        if cnn == 'resnet':
            self._cnn = ImageEncoderResnet(
                cnn_depth, cnn_blocks, resize, **cnn_kw)
        else:
...
class MultiDecoder(nj.Module):
    ...
    def __call__(self, inputs, drop_loss_indices=None):
        ...
        if self.cnn_shapes:
            ...
            split_indices = np.cumsum([v[-1]
                                      for v in self.cnn_shapes.values()][:-1])
    ...
    def __call__(self, x):
        ...
        for i in range(stages):
            ...
            if i == stages - 1:
                ...
                depth = self._shape[-1]
            if self._resize == 'stride':
                x = self.get(f's{i}res', Conv2D, depth,
                             4, 2, transp=True, **kw)(x)
            elif self._resize == 'stride3':
                s = 3 if i == stages - 1 else 2
                k = 5 if i == stages - 1 else 4
                x = self.get(f's{i}res', Conv2D, depth,
                             k, s, transp=True, **kw)(x)
...
class Dist(nj.Module):
    ...
    def inner(self, inputs):
        ...
        if self._dist in ('normal', 'trunc_normal'):
            std = self.get('std', Linear, int(
                np.prod(self._shape)), **kw)(inputs)
...
class Linear(nj.Module):
    ...
    def __call__(self, x):
        ...
        x = x @ kernel
        if self._bias:
            bias = self.get('bias', jnp.zeros, np.prod(
                self._units), np.float32)
            bias = jaxutils.cast_to_compute(bias)
            x += bias
