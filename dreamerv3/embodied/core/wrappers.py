...
class InfoWrapper(base.Wrapper):
    def __init__(self, env):
        super().__init__(env)
        self._support_types = (np.integer, np.floating, int, float)
    ...
    def _convert(self, value):
        if isinstance(value, bool):
            ...
            elif np.issubdtype(value.dtype, np.integer) or np.issubdtype(value.dtype, np.floating):
                return value
            else:
                return None
        elif isinstance(value, self._support_types):
            return value
        else:
            return None
    ...
    def step(self, action):
        ...
        for key, value in info.items():
            ...
            if converted_value is not None:
                info_filtered[key] = converted_value
...
class ExpandScalars(base.Wrapper):
    ...
    def step(self, action):
        ...
        info = {key: np.expand_dims(value, 0) if key in self._obs_expanded else value for key, value in info.items()}
        return obs, info
    ...
    def __init__(self, env):
        ...
        for key, space in self.env.obs_space.items():
            if len(space.shape) == 2:
                space = spacelib.Space(space.dtype, (int(np.prod(space.shape)),), space.low.flatten(), space.high.flatten())
...
class CheckSpaces(base.Wrapper):
    ...
    def _check(self, value, space, key):
        ...
        raise ValueError(f"Value for '{key}' with dtype {dtype}, shape {shape}, " f"lowest {lowest}, highest {highest} is not in {space}.")
