...
COLORS = {
    "contrast": ("#0022ff", "#33aa00", "#ff0011", "#ddaa00", "#cc44dd", "#0088aa", "#001177", "#117700", "#990022", "#885500", "#553366", "#006666"),
    "gradient": ("#a0da39", "#4ac16d", "#277f8e", "#365c8d", "#46327e", "#440154"),
    "gradient_more": ("#fde725", "#a0da39", "#4ac16d", "#1fa187", "#277f8e", "#365c8d", "#46327e", "#440154"),
}
...
def main():
    ...
    for directory in args.indirs:
        ...
        runs += load_metrics(
            directory, args.pattern, args.xaxis, args.yaxis, args.yaxis2, seed_prefix, method_prefix, args.tasks, args.methods, args.workers
        )
    tasks = []
    ...
    with gzip.open(filename, "w") as f:
        f.write(json.dumps([{**run, "xs": run["xs"].tolist(), "ys": run["ys"].tolist()} for run in runs]).encode("utf-8"))
    ...
    for task, ax in zip(tasks, axes):
        for i, method in enumerate(methods):
            ...
            for group in groups:
                ...
                curve(ax, xs, mean, mean - std, mean + std, label=args.labels.get(method, method), order=i, color=args.colors(i))
...
def compute_stats(runs, stats, bins):
    ...
    for stats in stats:
        if stats == "tasks":
            ...
            mins = select(baselines, "random")
            maxs = select(baselines, "human_record")
            extra_runs += stats_fixed_norm(runs, bins, mins, maxs, "record_mean_clip", lambda x, a: np.nanmean(np.minimum(x, 1), a))
        elif stats == "dmlab_mean":
            path = pathlib.Path("~/scores/dmlab_baselines.json").expanduser()
            baselines = json.loads(path.read_text())
            mins = select(baselines, "random")
            maxs = select(baselines, "human")
            extra_runs += stats_fixed_norm(runs, bins, mins, maxs, "human_mean", lambda vals, axis: np.nanmean(np.minimum(vals, 1), axis))
        else:
    ...
    for method in methods:
        for seed in seeds:
            ...
            if scores:
                ...
                extra_runs.append({"task": f"stats_normalized_{name}", "method": method, "seed": seed, "xs": xs, "ys": reduce(scores, aggregator, 0)})
    return extra_runs
...
def stats_fixed_norm(runs, bins, mins, maxs, name="mean", aggregator=np.nanmean):
    ...
    for method in methods:
        for seed in seeds:
            ...
            if scores:
                xs = np.linspace(0, 1, len(scores[0]))
                extra_runs.append({"task": f"stats_{name}", "method": method, "seed": seed, "xs": xs, "ys": reduce(scores, aggregator, 0)})
    return extra_runs
    ...
    for method in methods:
        for seed in seeds:
            ...
            if nonempty:
                ...
                extra_runs.append({"task": "stats_number_of_tasks", "method": method, "seed": seed, "xs": xs, "ys": np.sum(nonempty, 0)})
    return extra_runs


def load_metrics(directory, pattern, xaxis, yaxis, yaxis2, seed_prefix=None, method_prefix=None, tasks=(r".*",), methods=(r".*",), workers=1):
    console = rich.console.Console()
...
def curve(ax, xs, ys, low=None, high=None, label=None, order=0, **kwargs):
    ...
    if low is not None and finite.sum() > 1:
        ax.fill_between(xs[finite], low[finite], high[finite], zorder=100 - order, alpha=0.2, lw=0, **kwargs)


def legend(fig, mapping=None, adjust=False, **kwargs):
    options = dict(fontsize="medium", numpoints=1, labelspacing=0, columnspacing=1.2, handlelength=1.5, handletextpad=0.5, ncol=4, loc="lower center")
...
def natsort(sequence):
    ...
    return sorted(sequence, key=lambda x: [(int(y) if y.isdigit() else y) for y in pattern.split(x)])
