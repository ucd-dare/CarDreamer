...
def get_visibility(
    ...
    for id, poly in actor_polys.items():
        if id == ego_id:
            continue
        fov_visible[id] = is_fov_visible((ego_location.x, ego_location.y), ego_yaw, ego_id, id, poly, actor_polys, fov)
    ...
    for obs_id, vis in fov_visible.items():
        ...
        for id, poly in actor_polys.items():
            if id == obs_id or fov_visible[id] or recursive_visible[id]:
                continue
            if is_fov_visible((obs_location.x, obs_location.y), obs_yaw, obs_id, id, poly, actor_polys, fov):
