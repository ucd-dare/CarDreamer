...
class MapRenderer:
    ...
    def get_lane_markings(
        self, marking_type: carla.LaneMarkingType, marking_color: carla.LaneMarkingColor, waypoints: List[carla.Waypoint], sign: int
    ) -> List[Tuple[carla.LaneMarkingType, Color, List[Tuple[int, int]]]]:
        margin = 0.25
        marking_1 = np.array([self.world_to_pixel(lateral_shift(w.transform, sign * w.lane_width * 0.5)) for w in waypoints], dtype=np.int32)
        if marking_type == carla.LaneMarkingType.Broken or marking_type == carla.LaneMarkingType.Solid:
            ...
            marking_2 = np.array(
                [self.world_to_pixel(lateral_shift(w.transform, sign * (w.lane_width * 0.5 + margin * 2))) for w in waypoints], dtype=np.int32
            )
    ...
    def render_traffic_signs(self):
        ...
        cv2.putText(stop_font_surface, "STOP", (0, self._font_size * 2), self._font, 1, Color.ALUMINIUM_2, 2, cv2.LINE_AA)

        yield_font_surface = np.zeros((self._font_size * 2, self._font_size * 4, 3), dtype=np.uint8)
        cv2.putText(yield_font_surface, "YIELD", (0, self._font_size * 2), self._font, 1, Color.ALUMINIUM_2, 2, cv2.LINE_AA)
    ...
    def render_traffic_sign(self, actor: carla.Actor, font_surface: np.ndarray, trigger_color: Color):
        ...
        self._surface[offset[1] : offset[1] + font_surface.shape[0], offset[0] : offset[0] + font_surface.shape[1]] = font_surface
