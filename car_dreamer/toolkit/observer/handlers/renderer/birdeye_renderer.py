...
class BirdeyeRenderer:
    def __init__(
        ...
        self._command_initial = {Command.LaneFollow: "^", Command.LaneChangeLeft: "<", Command.LaneChangeRight: ">"}
    ...
    def _render_messages(self, **env_state):
        ...
        def render_character(location, message, message_color):
            ...
            cv2.putText(self._surface, message, (text_x, text_y), self._font, font_scale, message_color, font_thickness, cv2.LINE_AA)
    ...
    def _blit_centered(self, display: np.ndarray):
        center_offset = (max(0, (display.shape[1] - self._map_size) // 2), max(0, (display.shape[0] - self._map_size) // 2))
        display[
        ...
        ] = self._surface[: min(self._map_size, display.shape[0]), : min(self._map_size, display.shape[1])]
    ...
    def _render_polygon(self, surface: np.ndarray, vehicle_polygon: ActorPolygon, color: Color):
        ...
        actor_corners = np.array([self._world_to_pixel(carla.Location(x=p[0], y=p[1])) for p in vehicle_polygon], dtype=np.int32)
    ...
    def _render_path(self, surface: np.ndarray, vehicle_polygon: ActorPolygon, path: List[Tuple[float, float]], color: Color):
        ...
        corners = np.array([self._world_to_pixel(carla.Location(x=p[0], y=p[1])) for p in [polygon_center] + path], dtype=np.int32)
