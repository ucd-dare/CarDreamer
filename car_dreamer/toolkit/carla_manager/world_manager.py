...
class WorldManager:
    ...
    def try_spawn_actor(
        ...
        if blueprint is None:
            blueprint = self.get_blueprint("vehicle.audi*", {
                'number_of_wheels': '4'
            })
    ...
    def spawn_auto_actors(
        ...
        if blueprints is None:
            blueprints = self.get_blueprint_library("vehicle.*", {
                'number_of_wheels': '4'
            })
        batch = []
        actor_list = []
        ...
        for transform in transforms[:min(n, len(transforms))]:
            ...
            batch.append(
                carla.command.SpawnActor(bp, transform).then(
                    carla.command.SetAutopilot(carla.command.FutureActor, True, self._tm_port)
                )
            )
    ...
    def _get_actor_actions(self) -> ActorActionDict:
        ...
        for actor in self.actor_dict.values():
            try:
                ...
                actor_actions[actor.id] = [
                    (Command(command), waypoint) for command, waypoint in actions
                ]
