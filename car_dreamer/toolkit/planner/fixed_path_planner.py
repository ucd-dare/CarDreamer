...
class FixedPathPlanner(BasePlanner):
    ...
    def __init__(
        self, vehicle: carla.Actor, vehicle_path: List[Tuple[float, float, float]], use_road_waypoints: List[bool] = None, sampling_radius=0.8
    ):
        ...
        for i, start in enumerate(self._vehicle_path[:-1]):
            ...
            if self._use_road_waypoints is not None and self._use_road_waypoints[i]:
                segment_waypoints = self._grp.trace_route(
                    carla.Location(x=start[0], y=start[1], z=start[2]), carla.Location(x=end[0], y=end[1], z=end[2])
                )
                if i > 0:
