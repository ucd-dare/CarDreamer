    ...
    * ``early_lane_change``: The reward for early lane change.
    ...
    def get_nonego_vehicle_control(self):
        ...
        if dist < swing_trigger_dist:
            ...
            self.prev_errors = {'last_error': 0.0, 'integral': 0.0}  # Reset the prev_error
        else:
    ...
    def reward(self):
        ...
        if ego_y - self._config.reward.early_lane_change_dist < self.nonego.get_transform().location.y and ego_y + self._config.reward.reward_overtake_dist > self.nonego.get_transform().location.y:
        ...
        if ego_y - self._config.reward.early_lane_change_dist > self.nonego.get_transform().location.y and abs(ego_x -nonego_spawn_x) > self._config.reward.lane_width:
            p_early_lane_change = -reward_scales['early_lane_change']

        # Exceeding reward
        r_exceeding = 0.0
        if ego_y < self.nonego.get_transform().location.y and self.exceeding == False:
            r_exceeding = reward_scales["exceeding"]
            self.exceeding = True

        # Overtake reward (exceed and come back to the same lane)
        r_overtake = 0.0
        if ego_y + self._config.reward.reward_overtake_dist < self.nonego.get_transform().location.y and abs(ego_x -nonego_spawn_x) < self._config.terminal.lane_width / 5 and self.overtake == False:
    ...
    def get_terminal_conditions(self):
        ...
        info['out_of_lane'] =self.get_wpt_dist(ego_location) > terminal_config.out_lane_thres or ego_x < terminal_config.left_lane_boundry or ego_x > terminal_config.right_lane_boundry
        return info
